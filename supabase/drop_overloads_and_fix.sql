-- Drop the old overloaded UUID functions so PostgREST has exactly one unambiguous function
DROP FUNCTION IF EXISTS public.assign_complaint(uuid, uuid, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.suspend_user(uuid, text) CASCADE;
DROP FUNCTION IF EXISTS public.reactivate_user(uuid) CASCADE;

-- 1. Unambiguous assign_complaint (accepts text, handles UUIDs & public_ids like NAG-2026-000033)
